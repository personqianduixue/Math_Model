...
S = MODEL.addVars(I, K, N, vtype=gurobipy.GRB.BINARY, name="S")
...
# 创建目标函数
MODEL.setObjectiveN(-sum(R.iat[i - 1, j - 1] * y[i, j] for i in I for j in J), priority=2, index=0)
MODEL.setObjectiveN(-sum(X_[i, j] for i in I for j in J), priority=1, index=1)
MODEL.setObjectiveN(sum(S[i, k, n] for i in I for k in K for n in N), priority=0, index=2)
...
MODEL.addConstrs(sum(2 * (1 - x[i1, j, k, n]) for i1 in I1 for j in J) >= sum(x[i2, j, k + 1, n] + x[i2, j, k - 1, n] for i2 in (I2 | I3) for j in J) for k in K[1: -1] for n in N)
MODEL.addConstrs(sum(2 * (1 - x[i1, j, k, n]) for i1 in I2 for j in J) >= sum(x[i2, j, k + 1, n] + x[i2, j, k - 1, n] for i2 in (I1 | I3) for j in J) for k in K[1: -1] for n in N)
MODEL.addConstrs(sum(2 * (1 - x[i1, j, k, n]) for i1 in I3 for j in J) >= sum(x[i2, j, k + 1, n] + x[i2, j, k - 1, n] for i2 in (I1 | I2) for j in J) for k in K[1: -1] for n in N)
print(get_time(), "约束 9: I1, I2, I3不同项目零件不能相邻放置, 创建完成...")

MODEL.addConstrs(sum(2 * (1 - x[i1, j, k, n]) for i1 in I1 | I2 for j in J) >= sum(x[i2, j, k + 1, n] + x[i2, j, k - 1, n] for i2 in I4 for j in J) for k in K[1: -1] for n in N)
...
MODEL.addConstrs(sum(x[i, j1, k, n] for j1 in J) <= sum(x[i, j2, k, n + 1] for j2 in J) + S[i, k, n] for i in I for k in K for n in N[:-1])
print(get_time(), "约束 12: (2) 尽量避免圈与圈之间同个滑橇更换支架类型, 创建完成...")

# 执行最优化
MODEL.Params.TimeLimit = 6000    # 限制求解时间
MODEL.optimize()
...
# 输出模型结果
Excel_Writer = pd.ExcelWriter("../table/问题二模型求解.xlsx")
