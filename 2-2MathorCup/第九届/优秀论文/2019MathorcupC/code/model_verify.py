...
# 创建目标函数
MODEL.setObjective(sum(R.iat[i - 1, j - 1] * y[i, j] for i in I for j in J), gurobipy.GRB.MAXIMIZE)
...
# 执行最优化
MODEL.Params.TimeLimit = 2000    # 限制求解时间
MODEL.optimize()
...
# 输出模型结果
Excel_Writer = pd.ExcelWriter("../table/模型适用性验证.xlsx")
